...
@property(nonatomic,strong)AAPLEAGLLayer *playLayer;//用于竖屏播放
...
-(void)start{
    ...
    if ([self.delegate respondsToSelector:@selector(parsingStartPlay)]) {
        [self.delegate parsingStartPlay];
    }
}
...
-(void)end{
    ...
    sockState = SOCKETCUT;
    _socket = nil;
    self.imgv.image = [UIImage new];
    if ([self.delegate respondsToSelector:@selector(parsingStopPlay)]) {
        [self.delegate parsingStopPlay];
    }
}
//初始化解码器,录音  音频解码播放初始化
- (void)initDecoder {
...
//                NSLog(@"数据错误断开%d",len);
                ...
                if (!playState) {
                    playState = YES;
                    dispatch_async(dispatch_get_main_queue(), ^{
                        if ([self.delegate respondsToSelector:@selector(parsingPlaySuccess)]) {
                                [self.delegate parsingPlaySuccess];
